  ...
  <span>The Execution Roadmap</span>
        ...
        <div className="flex flex-col lg:flex-row gap-6 sm:gap-8 lg:gap-12 min-h-[600px]">
          ...
          <div className="lg:w-1/3 flex flex-col gap-4 sm:gap-5">
            {phases.map((phase) => (
              <button
                ...
                className={`w-full text-left p-4 sm:p-5 rounded-xl sm:rounded-2xl flex items-center gap-3 sm:gap-4 group outline-none ${
              ...
              >
                ...
                <div className="flex-1">
                  <div className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">{phase.week}</div>
                  <div className={`font-bold text-xs sm:text-sm ${activePhase === phase.id ? 'text-slate-800' : 'text-slate-600'}`}>
                    {phase.title}
                  </div>
                </div>
          ...
          <div className="lg:w-2/3">
            <div className="h-full rounded-2xl sm:rounded-[2.5rem] neu-flat p-6 sm:p-8 md:p-12 relative overflow-hidden flex flex-col">
              
              {/* Decorative Elements */}
              <div className="absolute top-0 right-0 w-48 h-48 sm:w-64 sm:h-64 bg-slate-100/50 rounded-full blur-[60px] -z-10"></div>
              
              {/* Content Header */}
              <div className="mb-6 sm:mb-8 border-b border-slate-200/60 pb-4 sm:pb-6">
                <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                  <span className={`px-2.5 sm:px-3 py-1 rounded-lg bg-${activeData.color}-50 text-${activeData.color}-600 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest border border-${activeData.color}-100`}>
                    {activeData.week} Phase
                  </span>
                </div>
                <h3 className="text-2xl sm:text-3xl font-bold text-slate-700 mb-3 sm:mb-4">{activeData.title}</h3>
                <p className="text-slate-500 leading-relaxed text-sm sm:text-base md:text-lg">{activeData.desc}</p>
              </div>

              {/* Steps & Technicals */}
              <div className="flex-grow space-y-4 sm:space-y-6">
                <div>
                  <h4 className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 sm:mb-4">Execution Steps</h4>
                  <ul className="space-y-2 sm:space-y-3">
                    {activeData.steps.map((step, i) => (
                      <li key={i} className="flex items-start gap-2 sm:gap-3 text-xs sm:text-sm text-slate-600 font-medium">
                        <CheckCircle2 size={16} className={`text-${activeData.color}-500 flex-shrink-0 mt-0.5 sm:w-[18px] sm:h-[18px]`} />
                        {step}
                      </li>
                    ))}
                  </ul>
                ...
                {/* AI Comparison Block (Only for Phase 2) */}
                {activeData.comparison && (
                  <div className="mt-4 sm:mt-6 p-3 sm:p-4 rounded-xl neu-pressed">
                    <div className="grid grid-cols-2 gap-3 sm:gap-4">
                      <div className="p-2.5 sm:p-3 rounded-lg bg-red-50/50 border border-red-100 opacity-60">
                        <div className="text-[9px] sm:text-[10px] uppercase font-bold text-red-400 mb-1">Traditional Agency</div>
                        <div className="text-base sm:text-lg font-bold text-slate-500 line-through">$10,000</div>
                        <div className="text-[9px] sm:text-[10px] text-slate-400">4 Weeks</div>
                      </div>
                      <div className="p-2.5 sm:p-3 rounded-lg bg-green-50/50 border border-green-100 transform scale-105 shadow-sm">
                        <div className="text-[9px] sm:text-[10px] uppercase font-bold text-green-600 mb-1 flex items-center gap-1">
                          <Zap size={10}/> Arlox AI
                        </div>
                        <div className="text-base sm:text-lg font-bold text-slate-700">$1,500</div>
                        <div className="text-[9px] sm:text-[10px] text-slate-500">5 Days</div>
                      </div>
                    </div>
                  </div>
                )}
              ...
              {/* Footer Metrics */}
              <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-slate-200/60 flex flex-wrap gap-3 sm:gap-4 md:gap-8 items-center">
                <div className="flex-1 min-w-[100px] sm:min-w-[120px]">
                  <div className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase mb-1">Deliverable</div>
                  <div className="text-[11px] sm:text-xs font-bold text-slate-700">{activeData.deliverable}</div>
                </div>
                <div className="flex-1 min-w-[100px] sm:min-w-[120px]">
                  <div className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase mb-1">Your Involvement</div>
                  <div className="text-[11px] sm:text-xs font-bold text-slate-700">{activeData.involvement}</div>
                </div>
              </div>
            </div>
          </div>
